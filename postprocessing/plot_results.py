...
def show_evolution_nolegend(sample_img, dataset, model, nonwater=0, water=1, water_threshold=0.5, pixel_size=60, device='cuda:0', 
    ...
    # get target year
    year, year2 = [1988 + i for i in range(2)], [2000 + i  for i in range(18)] # update if 2021 prediction is made
    year = year + year2
    ...
    ax[1,3].set_xticks(bar_positions)  # Corrected: Remove fontsize here
    ax[1,3].set_xticklabels(categories, fontsize=12)  # Corrected: Apply fontsize to tick labels
...
def total_losses_metrics_dataset_max(model, dataset, loss_f='BCE', nonwater=0, water=1, water_threshold=0.5, overall=True, model_type='min loss', device='cuda:0'):
    '''
...
def total_losses_metrics_dataset_min(model, dataset, loss_f='BCE', nonwater=0, water=1, water_threshold=0.5, device='cuda:0'):
    '''
    Get loss and metrics for each sample of the given dataset
